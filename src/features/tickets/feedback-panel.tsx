...
const typeOptions = Object.entries(feedbackTypeLabels).map(
  ([value, label]) => ({
...
export function FeedbackPanel({
  ...
  const activeTicket =
    initialTickets.find((ticket) => ticket.id === activeTicketId) ??
    initialTickets[0];
    ...
    <div className="grid gap-5 lg:grid-cols-[22rem_minmax(0,1fr)]">
      ...
      <section className="surface rounded-xl p-5">
        ...
        {initialTickets.length === 0 ? (
          <div className={styles.feedbackPanel__empty}>
            还没有反馈记录，提交后会显示在这里。
          </div>
        ) : (
                  ...
                  {activeTicket.messages.map((item) => (
                    ...
                    >
                      <p className="text-xs font-medium text-muted">
                        {item.isAdmin ? "管理员" : item.authorName} /{" "}
