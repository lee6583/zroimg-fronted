...
import { getJavaRechargeOrder } from "@/server/bff/orders";
...
async function getMockOrder(orderNo: string, userProfileId: string) {
...
async function getJavaOrder(orderNo: string) {
  try {
    const order = await getJavaRechargeOrder(orderNo);
    return jsonOk(order);
  } catch (error) {
...
export async function GET(_request: Request, context: { params: Promise<{ orderNo: string }> }) {
  ...
  if (isMockBffEnabled()) {
    return getMockOrder(orderNo, current.profile.id);
  }

  return getJavaOrder(orderNo);
}
