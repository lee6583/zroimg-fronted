    ...
    <AdminShell active="users">
      <div className="grid gap-6">
        <section>
          ...
          <h1 className="mt-2 font-serif text-4xl font-medium tracking-tight md:text-5xl">用户管理</h1>
        ...
        <section className="surface rounded-xl p-5">
          <form className="grid gap-3 md:grid-cols-[1fr_auto_auto_auto]">
            <input className="field text-sm" name="q" defaultValue={q || ""} placeholder="搜索用户名或邮箱" />
            <Link className={`btn-secondary ${!status ? "bg-soft" : ""}`} href="/admin/users">
              全部
            </Link>
            <Link className={`btn-secondary ${status === "active" ? "bg-soft" : ""}`} href={pageHref({ q, status: "active", page: 1 })}>
              正常
            </Link>
            <Link className={`btn-secondary ${status === "banned" ? "bg-soft" : ""}`} href={pageHref({ q, status: "banned", page: 1 })}>
              封禁
            </Link>
        ...
        <section className="grid gap-3">
          {users.map((profile) => (
            <article key={profile.id} className="surface rounded-xl p-4">
              <div className="mb-3 flex flex-wrap justify-between gap-3">
                <div>
                  <Link href={`/admin/users/${profile.id}`} className="font-semibold transition hover:text-muted">
                    {profile.username}
                  </Link>
                  ...
                  <p className="mt-1 text-xs text-muted">
                    任务 {profile._count?.tasks || 0} / 订单 {profile._count?.paymentOrders || 0} / 流水 {profile._count?.creditLedger || 0}
                  </p>
                </div>
          ...
          {users.length === 0 ? <p className="surface rounded-xl p-8 text-center text-sm text-muted">没有匹配用户</p> : null}
        </section>

        <nav className="flex items-center justify-between">
          <Link className="btn-secondary" href={pageHref({ q, status, page: Math.max(1, page - 1) })}>
            上一页
          </Link>
          <p className="text-sm text-muted">
            第 {page} / {totalPages} 页，共 {total} 位用户
          </p>
          <Link className="btn-secondary" href={pageHref({ q, status, page: Math.min(totalPages, page + 1) })}>
            下一页
          </Link>
        </nav>
      </div>
