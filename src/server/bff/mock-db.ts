...
export const prisma = {
  generatedImage: {
    async count(args?: { where?: { userProfileId?: string } }) {
      const store = getStore();
      return store.generatedImages.filter((item) =>
        args?.where?.userProfileId ? item.userProfileId === args.where.userProfileId : true,
      ).length;
    },
  },
  creditPackage: {
    async findMany(args?: {
      ...
      return [...store.creditPackages]
        .filter((item) =>
          typeof args?.where?.isActive === "boolean" ? item.isActive === args.where.isActive : true,
        )
        .sort((a, b) => {
  ...
  paymentOrder: {
    ...
    async findMany(args?: {
      ...
      items = items.filter((order) => {
        if (args?.where?.userProfileId && order.userProfileId !== args.where.userProfileId)
          return false;
        if (args?.where?.status && order.status !== args.where.status) return false;
        if (args?.where?.OR?.length) {
          const query =
            args.where.OR[0]?.orderNo?.contains ||
            args.where.OR[1]?.providerTradeNo?.contains ||
            args.where.OR[2]?.userProfile?.user?.email?.contains;
          const { user } = userByProfileId(order.userProfileId);
          if (
            !includesText(order.orderNo, query) &&
            !includesText(order.providerTradeNo, query) &&
      ...
      return items.slice(start, end).map((order) => {
        const foundPackage = store.creditPackages.find((item) => item.id === order.creditPackageId);
  ...
  userProfile: {
    ...
    async findMany(args?: {
      ...
      const query =
        args?.where?.OR?.[0]?.username?.contains || args?.where?.OR?.[1]?.user?.email?.contains;
      ...
      items = items.filter((profile) => {
        ...
        if (query && !includesText(profile.username, query) && !includesText(user.email, query))
          return false;
        return true;
      });
      ...
      return items.slice(start, end).map((profile) => {
        ...
        return {
          ...
          ...(args?.include?._count
            ? {
                _count: {
                  tasks: store.generationTasks.filter((task) => task.userProfileId === profile.id)
                    .length,
                  paymentOrders: store.paymentOrders.filter(
                    (order) => order.userProfileId === profile.id,
                  ).length,
                  creditLedger: store.creditLedger.filter(
                    (ledger) => ledger.userProfileId === profile.id,
                  ).length,
                },
              }
            : {}),
  ...
  generationTask: {
    ...
    async findMany(args?: {
      ...
      const store = getStore();
      const query =
        args?.where?.OR?.[0]?.prompt?.contains ||
        args?.where?.OR?.[1]?.model?.contains ||
        args?.where?.OR?.[2]?.userProfile?.user?.email?.contains;
      const items = [...store.generationTasks].filter((task) => {
        const { user } = userByProfileId(task.userProfileId);
        if (args?.where?.userProfileId && task.userProfileId !== args.where.userProfileId)
          return false;
        if (args?.where?.conversationId && task.conversationId !== args.where.conversationId)
          return false;
        if (args?.where?.status && task.status !== args.where.status) return false;
        if (
          query &&
          !includesText(task.prompt, query) &&
          !includesText(task.model, query) &&
          !includesText(user.email, query)
        )
          return false;
        return true;
  ...
  feedbackTicket: {
    ...
    async findMany(args?: {
      ...
      const store = getStore();
      const items = [...store.feedbackTickets].sort((a, b) => {
